async function main() {
  try {
    ...
    const parserMinYear = Number(
      process.env.PARSER_MIN_YEAR ? process.env.PARSER_MIN_YEAR : 2000
    );
    try {
    ...
    if (String(process.env.PARSER_SCHEDULE_ORDERS).toLowerCase() != "false") {
      ...
      console.log(
        "[INFO] Parser Orders launch schedule set at <" +
          process.env.PARSER_SCHEDULE_ORDERS +
          ">"
      );
    }
    ...
    if (String(process.env.PARSER_SCHEDULE_ACTS).toLowerCase() != "false") {
      ...
      console.log(
        "[INFO] Parser Acts&Orders launch schedule set at <" +
          process.env.PARSER_SCHEDULE_ACTS +
          ">"
      );
    }
    ...
    if (String(process.env.PARSER_SCHEDULE_OATH).toLowerCase() != "false") {
      try {
        crontab.schedule(process.env.PARSER_SCHEDULE_OATH, () => {
          ...
          parser.runParseOath(async (application) => {
            await informSubscribers(
              application,
              async (application, subscriber) => {
                application.subscribers.splice(
                  application.subscribers.indexOf(subscriber),
                  1
                );
      ...
      console.log(
        "[INFO] Parser Oath launch schedule set at <" +
          process.env.PARSER_SCHEDULE_OATH +
          ">"
      );
    }
    ...
    const botSettings =
      String(process.env.BOT_USE_ONLY_ENV_TOKEN).toLowerCase() == "true"
        ...
        : (await BotSettings.findOne()) ||
          (await BotSettings.create({ token: process.env.BOT_TOKEN }));
    ...
    bot.hears(/(^\d{4}(\/| )\d+$)/, async (ctx) => {
      ...
      if (
        !(Number(appYear) >= parserMinYear) &&
        Number(appYear) <= Number(new Date().getFullYear())
      ) {
      ...
      return (await sendApplicationInfo(application, ctx)) && ctx.session.owner
        ? await ctx.replyWithHTML(
            ctx.i18n.t("adConsultation", { owner: ctx.session.owner })
          )
        : true;
    });
    ...
    const sendApplicationInfo = async (application, ctx) => {
      try {
        ...
        const { appNumber, appYear } =
          /(?<appNumber>\d+)(?<appYear>\d{4})/.exec(application.index).groups;
        const messageData = {
          appNumber,
          appYear,
        ...
        if (application.oathDateTime)
          ...
          message = ctx.i18n.t("applicationInfo_consideration", messageData);
        else if (application.registrationDate)
          message = ctx.i18n.t("applicationInfo_registration", messageData);
        return ctx.replyWithHTML(
          message,
          !application.oathDateTime
            ? !(
                application.subscribers &&
                application.subscribers.indexOf(ctx.from.id) + 1
              )
              ? Extra.HTML().markup((m) =>
                  m.inlineKeyboard([
                    m.callbackButton(
                      ctx.i18n.t("subscribeButton"),
              ...
              : Extra.HTML().markup((m) =>
                  m.inlineKeyboard([
                    m.callbackButton(
                      ctx.i18n.t("unSubscribeButton"),
                      "unsub" + application.index
                    ),
                  ])
                )
            : undefined
    ...
    const informSubscribers = async (
      application,
      callback = async (application, subscriber) => {}
    ) => {
      try {
        if (!application.subscribers || application.oathDateTime < Date.now())
          return;
        for (subscriber of application.subscribers) {
          ...
          try {
            ...
            const fs = require("fs");
            fs.writeFileSync("sendInfoCallback.txt", "Привет ми ми ми!");
          }
        }
      } catch (e) {
        console.log(e);
