...
const oathUrl = process.env.BOT_OATH_URL || 'http://cetatenie.just.ro/juramant/#1576651821062-52dd0897-6698';
...
async function getHtmlFromUrl(url = String()) {
  try {
    ...
    if (response.status != '200') throw new Error('Responce status ' + response.status);
...
async function getActsFromSite() {
  try {
    ...
    for await (const line of (await getHtmlFromUrl(actsUrl))
      .normalize('NFKD')
      .replace(/\s+/g, ' ')
      .match(/(id="1576832773102-627a212f-45ce").+?(<div class="vc_tta-panel( vc_active|)")/gs)[0]
      .match(/(?<=<li><a href=")(.+?)(?=<\/a>.+?<\/li>)/g)) {
      const { year, fileName, link } = /(?<link>.+?(?<fileName>(?<=\/)(.[^/]+).pdf))">(?<year>\d{4})/.exec(line).groups;
      acts.push({
        year: Number(year),
...
async function getOrdersFromSite() {
  try {
    ...
    for await (const line of (await getHtmlFromUrl(ordersUrl))
      .normalize('NFKD')
      .replace(/\s+/g, '')
      .match(/<li>.+?(\d{2}\.\d{2}\.\d{4}).+?<ahref="(.+?\.pdf)">(\d+\w)<\/a><\/li>/g)) {
      const { date, year, rawOrders } =
        /<li>.+?(?<date>\d{2}\.\d{2}\.(?<year>\d{4})).+?(?<rawOrders><ahref=".+?<\/a>)<\/li>/.exec(line).groups;
      for await (const rawOrder of rawOrders.match(/<ahref.+?>\d+\w<\/a>/g)) {
        const { link, fileName, name } =
          /<ahref="(?<link>.+?(?<fileName>(?<=\/)(.[^/]+)\.pdf))">(?<name>\d+\w)<\/a>/g.exec(rawOrder).groups;
...
async function updateActs(actsArray = [{ year: Number(), filePath: String(), link: String() }]) {
  try {
    ...
    if (acts.length != actsArray.length)
      console.log(`[INFO] Acts on site ${actsArray.length} | Acts in base ${acts.length}`);
    if (acts.length > actsArray.length) {
      console.log('[WARNING] Acts in DataBase more then Acts on the site. Removing orphans.');
      const actsYears = [];
    ...
    for await (const _act of actsArray) {
      ...
      if (!fs.existsSync(act.filePath)) {
        process.stdout.write(`\r[INFO] Downloading ${act.filePath.match(/(?<=\/)(.[^\/]+).pdf$/g)[0]} `);
...
async function updateOrders(
  ...
  try {
    ...
    if (orders.length != ordersArray.length)
      console.log(`[INFO] Orders on site ${ordersArray.length} | Orders in base ${orders.length}`);
    if (orders.length > ordersArray.length) {
      console.log('[WARNING] Orders in DataBase more then Orders on the site. Removing orphans.');
      const ordersIndexes = [];
    ...
    for await (const _order of ordersArray) {
      const order = (await Order.findOne({ index: _order.index })) || (await Order.create(_order));
      if (!fs.existsSync(_order.filePath)) filesToDownload.push({ link: _order.link, filePath: _order.filePath });
    }
    let count = 0;
    let result = true;
    for await (const line of filesToDownload) {
      count++;
      process.stdout.write(`\r[INFO] Downloading Orders files. ${count} of ${filesToDownload.length} `);
...
async function getApplicationArrayFromActPdf(act) {
  try {
    ...
    for await (const rawApp of (await pdf(fs.readFileSync(act.filePath))).text
      ...
      const index = (String(appData.number) + String(appData.year)).toUpperCase();
      applications[index] = {
        index,
...
async function getApplicationArrayFromOrderPdf(order) {
  try {
    ...
    console.error('[ERROR] getApplicationArrayFromOrderPdf ' + `\n${order.filePath}\n` + e);
    return false;
  }
}
...
async function getApplicationsFromActs(actRange = Number()) {
  ...
  for await (const act of Act.find(
    actRange ? { year: { $gt: (await Act.findOne().sort('-year')).year - actRange } } : {},
  )) {
    applications = {
  ...
  try {
    ...
    for (const oath of oaths) {
      for (const oathOrder of (await pdf(fs.readFileSync(oath.filePath))).text
        ...
        .toLowerCase()
        .match(/\d+\/\d{4}/g)) {
        const { orderNumber, orderYear } = /(?<orderNumber>\d+)\/(?<orderYear>\d{4})/.exec(oathOrder).groups;
        const applicationIndex = (String(orderNumber) + String(orderYear)).toLowerCase();
        const application = await Application.findOne({
          index: applicationIndex,
        });
        if (!application) {
          console.log(`[INFO] Application ${applicationIndex} doesn't exist. Creating...`);
    ...
    console.error('[ERROR] processOath ' + e);
  }
}
...
async function runParseOath(updateInformCallback = (application) => {}) {
  ...
  console.log('[INFO] Start parse oaths');
  await updateOath(await getOathFromSite());
  await processOath(updateInformCallback);
  console.log('[INFO] SUCCESS parse oaths');
}

async function run(updateInformCallback = (application) => {}, options = { actUpdateRange: Number() }) {
  try {
    if (typeof updateInformCallback !== 'function')
      updateInformCallback = (a) => {
        console.log(a);
    ...
    if (coldStart) {
      ...
      const applicationsInAct = options.actUpdateRange ? await getApplicationsFromActs(options.actUpdateRange) : {};
      const applicationsInOrders = await getApplicationsFromOrders();
      const applicationsInActIndexes = Object.keys(applicationsInAct);
      const applicationsInOrdersIndexes = Object.keys(applicationsInOrders);
      const updateIndexes = [...new Set(applicationsInActIndexes.concat(applicationsInOrdersIndexes).sort())];
